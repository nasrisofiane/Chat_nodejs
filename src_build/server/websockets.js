...
exports.sendLeavedChatMessageBroadcaster = exports.sendJoinedChatMessageBroadcaster = exports.default = void 0;

var _expressSocket = _interopRequireDefault(require("express-socket.io-session"));
...
let webSockets;
let usersInChat; //Enum equivalent.
...
const startWebsocketsApp = (server, session, database) => {
  webSockets = (0, _socket.default)(server);
  webSockets.use((0, _expressSocket.default)(session));
  /**
  ...
  webSockets.sockets.on('connection', socket => {
    ...
    socket.handshake.session.save(); //Function that retrieve few documents from a collection with a number of results and a callback function.

    database.actionToDatabase(database.getFewDocuments, 'messages', {
      limit: 4,
      searchByFields: {}
    }, results => {
    ...
    getConnectedUsers(socket); //last private message search criteria

    let searchLastConversations = {
      users: {
        $in: [username]
      }
    }; //Get lasts private messages

    database.actionToDatabase(database.getFewDocuments, 'privateConversations', {
      limit: 4,
      searchByFields: searchLastConversations
    }, results => {
      ...
      if (results.length) {
        //Send a message to the user session to retrieve lasts messages from the chat.
        socket.emit('lastConversations', results);
      }
    ...
    socket.on('messagesSeen', conversationId => {
      database.actionToDatabase(database.getFewDocuments, 'privateConversations', {
        ...
        if (results.length && results[0].users && results[0].users.includes(username)) {
          ...
          database.actionToDatabase(database.insertSingleDocument, 'privateConversations', {
            _id: conversationId,
            users: users,
            messages: messages
          }, insertResults => {
    ...
    socket.on('message', message => {
      ...
      if (username) {
        ...
        database.actionToDatabase(database.insertSingleDocument, 'messages', {
          username: username,
          message: message
        }, results => null);
      } else {
  ...
  const getConnectedUsers = (socket, callback) => {
    database.actionToDatabase(database.getFewDocuments, 'sessions', {
      limit: 0
    }, results => {
...
var _default = startWebsocketsApp;
